    ...
    class Program
    {
        ...
        static void CrearContratos()
        {
            using (var db = new CryptoContext())
            {
                ...
                db.Contrato.AddRange(
                   ...
                   new Contrato { ContratoId = 5, MonedaId = "ShibaInu", CarteraId = 2, Cantidad = 4 },
        ...
        static void ConsultarCriptoDB()
        {
            using (var db = new CryptoContext())
            {
                ...
                var list1 = db.Moneda.Where(m => m.Actual >= 50M).OrderBy(m => m.MonedaId).ToList();
                list1.ForEach(Console.WriteLine);

                // 1
                WriteLine("2.- Carteras con más de 2 monedas contratadas");
                var list2 = db.Cartera.Where(c => c.ContratosCartera.Count > 2).Select(c => new{
                    CarID = c.CarteraId,
                    Monedas = c.ContratosCartera.Count
                }).ToList();
                ...
                var list3 = db.Cartera.GroupBy(c => c.Exchange).Select(c => new
                {
                    Exchange = c.Key,
                    Car = c.Count()
                }).OrderByDescending(c => c.Car).ToList();
                list3.ForEach(Console.WriteLine);

                // 2
                WriteLine("4.- Exchanges ordenados por cantidad de monedas");
                var list4 = db.Cartera.GroupBy(c => c.Exchange).Select(c => new
                {
                    Exchange = c.Key,
                    Car = c.Count()
                }).OrderByDescending(c => c.Car).ToList();
                ...
                var list5 = db.Moneda.SelectMany(m => m.ContratosMoneda, (a,b) => new{
                    Moneda = a.MonedaId,
                    Contrato = a.MonedaId+" "+b.ContratoId,
                    ValorContrato = b.Cantidad * a.Actual
                ...
                var list6 = list5.GroupBy(m=>m.Moneda)
                .Select(m => new
                {
                    Moneda = m.Key,
                    ValorTotal = m.Sum(s => s.ValorContrato)
                }).OrderByDescending(m => m.ValorTotal).ToList();
                ...
                var list7 = db.Moneda.SelectMany(m => m.ContratosMoneda, (a,b) => new{
                    Moneda = b.MonedaId,
                    ValorContrato = a.Actual * b.Cantidad
                }).GroupBy(m => m.Moneda).Select(m => new{
                    Monedas = m.Key,
                    ValorDelContrato = m.Key,
                    Contratos = m.Count()
                }).ToList();
